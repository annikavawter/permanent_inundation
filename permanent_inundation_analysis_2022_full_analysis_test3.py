...
def prep_noaa_mhhw_layers (regions):  ## THE MAKING PART HAS NOT BEEN TESTED< MAKE NEW GDB OUT AND RUN
    for region in regions:
        ...
        if len(mhhw_mos_check)>0:
            ...
            arcpy.MosaicToNewRaster_management(mhhw_paths, gdb_out, outname_mhhw_mos,"","32_BIT_FLOAT","",1,"MAXIMUM","FIRST")   #cellsz_pt000045
...
def prep_gauge_data_for_interpolation (region,projection):  # need to test, with interp funct
    ...
    #print('END PREP GAUGE DATA SCRIPT')

def interpolate_and_create_water_level_surfaces (flood_frequency,regions,projections,years):  #WORKING, BREAKING ON WLS CREATION
    for region in regions:
        for projection in projections:
            ...
            for year in years:
                ...
                # interpolated_surface_wrt_mhhw = NaturalNeighbor(transect_points_layer,str('all_{0}_stations_{1}x_{2}_proj.F{3}_{4}' .format(region,flood_frequency,projection, year,projection) ), cellsize)
                interpolated_surface_wrt_mhhw = NaturalNeighbor(transect_points_layer,str('all_{0}_stations_{1}x_{2}_proj.{3}_{4}' .format(region,flood_frequency,projection,  projection,year) ) )
                ...
                # interpolate_and_create_water_level_surfaces.save(outname_interpol)
                # print('Saved interpolated surface')
                ...
                # water_level_surface = RasterCalculator( Raster(mhhw_layer) + Raster(interpolated_surface_wrt_mhhw) )
                # water_level_surface = RasterCalculator( ("mhhw_layer" + "interpolated_surface_wrt_mhhw") , 'water_level_surface' )
                # water_level_surface =  (Raster("mhhw_layer") + Raster("interpolated_surface_wrt_mhhw"))
                # water_level_surface =  "mhhw_layer" + "interpolated_surface_wrt_mhhw"  #NOT WORKING WON"T RASTER
                # print('Water level surface created')

                interp_rast = Raster(interpolated_surface_wrt_mhhw)
                mhhw_rast = Raster(mhhw_data)

                # water_level_surface_1 = Raster(interpolated_surface_wrt_mhhw) + Raster(mhhw_data)
                water_level_surface_1 = interp_rast + mhhw_rast
                print('Water level surface created')

                # water_level_surface_rast = Raster(water_level_surface)
                ...
                # water_level_surface_rast.save(outname_wls)
                water_level_surface_1.save(outname_wls)
                print('Layer saved')

                outfind = arcpy.ListRasters(outname_wls)
                print(len(outfind))
                
                print ('STATUS: interpolation run successfully for ' + region + ' ' + projection + ' ' + year)
                tf2 = datetime.datetime.now() ; print('Time start: ' + str(tf1) + '  &  Time end: ' + str(tf2)) 
    ...
    for region in regions:
        for projection in projections:
            for year in years:
                ...
                # set path for saving inundated raster
                # gdb_out = 'C:/Users/annik/Documents/UCS/Data/2022/permanent_inundation/{0}/{0}.gdb' .format(region)
                gdb_out = 'C:/Users/annik/Documents/UCS/Data/2022/permanent_inundation/{0}/{0}_prime.gdb' .format(region) #gdb swap <--corrupted files
    ...
    for region in regions:
        for projection in projections:
            for year in years:
                ...
                gdb_in = 'C:/Users/annik/Documents/UCS/Data/2022/permanent_inundation/{0}/{0}_prime.gdb' .format(region)
                gdb_in_old = 'C:/Users/annik/Documents/UCS/Data/2022/permanent_inundation/{0}/{0}.gdb' .format(region)
                ...
                # set path out
                # gdb_out = 'C:/Users/annik/Documents/UCS/Data/2022/permanent_inundation/{0}/{0}.gdb' .format(region)
                gdb_out_scratch = 'C:/Users/annik/Documents/UCS/Data/2022/permanent_inundation/{0}/{0}.gdb' .format(region)
                gdb_out = gdb_in
                ...
                if region == 'east_coast':
                    ...
                    for area in areas:
                        ...
                        arcpy.env.workspace = gdb_in_old #shifting
                        ...
                        arcpy.env.workspace = gdb_in #shifting
                        ...
                        for raster_inund_file in raster_inund_files:
                            ...
                            rast_path = str(gdb_in_old) + '/' + str(raster_inund_file)
                            ...
                            outname_resample_path = str(gdb_in_old) + '/' + str(outname_resample)  

                            #delete old/weird resampled files
                            delweird=1
                            if delweird==1:
                                arcpy.env.workspace = gdb_in_old
                        ...
                        if runmos==1:
                            ...
                            if moscreated==1:
                                ...
                                arcpy.env.workspace = gdb_out_scratch
    ...
    for region in regions:
        for projection in projections:
            for year in years:
                ...
                # gdb_out = 'C:/Users/annik/Documents/UCS/Data/2022/permanent_inundation/{0}/{0}.gdb' .format(region)
                gdb_in = 'C:/Users/annik/Documents/UCS/Data/2022/permanent_inundation/{0}/{0}_prime.gdb' .format(region)
                gdb_out = 'C:/Users/annik/Documents/UCS/Data/2022/permanent_inundation/{0}/{0}_prime.gdb' .format(region)
...
#PROBLEMS: 2065 IL fixed; 2050 H ; 2100 H ; 2100 IL
...
# region_group          ('26', ['east_coast'], ['int_low'], ['2065'])  #running all below
# extract_connected     ('26', ['east_coast'], ['int_low'], ['2065'])
# raster_to_polygon     ('26', ['east_coast'], ['int_low'], ['2065'])


#dont do stuff with below until import inund files into prime gdb
## inundated_areas_select_dems ('26', ['east_coast'], ['high'], ['2050'], ['MD_Southwest_GCS_3m_NAVDm']) #not run, checking

# inundated_areas ('26', ['east_coast'], ['high'], ['2050']) #to scratch

# mosaic_chunks         ('26', ['east_coast'], ['high'], ['2050'], ['atlantic_area']) #RUNNING

# mosaic_chunks         ('26', ['east_coast'], ['high'], ['2100']) #problems gulf and atl , not run


#mosaic_chunks         ('26', ['east_coast'], ['high'], ['2050']) #atlan only is being a problem child
## region_group          ('26', ['east_coast'], ['high'], ['2050'])
## extract_connected     ('26', ['east_coast'], ['high'], ['2050'])
## raster_to_polygon     ('26', ['east_coast'], ['high'], ['2050'])

# mosaic_chunks         ('26', ['east_coast'], ['high'], ['2100']) #all below not run
